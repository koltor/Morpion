...
class Player
	...
	attr_accessor :nom, :forme, :etat, :type
...
class Game
		...
		def initialize(a = 1)
			...
			tab = ["jean", "michel", "patrick" , "maxence", "bilel", "chloe", "kevin", "seb", "paul"]
			i = a
			j = 0 
			if i > 1
				k = rand(0...tab.length)
				nom = 
				player1 = Player.new(tab[k],"X","y","ia")
			end
			if i == 3
				kn = 0
				while  kn == k
					kn = rand(0...tab.length)
				end
				player2 = Player.new(tab[kn],"O","O","ia")
			end
				while i <= 2
					...
					if i == 1
						...
						etat = gets.chomp()
						player1 = Player.new(nom,forme,etat,"perso")
					else
						if etat == "O"
							etat = "y"
							j = 1
						...
						player2= Player.new(nom,forme,etat,"perso")
					end	
					i += 1
				end
